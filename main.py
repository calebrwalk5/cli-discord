...
parser = ArgumentParser(description='CLI Discord', usage='main.py -t token [-c CHANNEL] [-h]')
parser.add_argument('-t', '--token', help='Your discord token')
parser.add_argument('-c', '--channel', help='The channel to start out in', type=int)
...
if float('.'.join(platform.python_version().split('.')[:2])) < 3.5:
    cprint('\n'.join((
        'You are using an unsupported version of Python, loser. UPGRADE NOW.',
        'UPGRADE TO PYTHON 3.5+ RETARD',
        'You are currently using ' + platform.python_version()
    )), 'red')
    exit(0)
...
class Bot(commands.Bot):
    ...
    def run(self):
        '''for the main method login shit, read it yourself bro'''
